...
Run these only after authorizing the particular installation and inspecting the current installer help.
...
| Playwright skills, after approved CLI installation | `playwright-cli install --skills` |
| Superpowers in Claude Code | `/plugin install superpowers@claude-plugins-official` |
| Superpowers in Codex | Open `/plugins`, find Superpowers, inspect and install |
...
For `gh-fix-ci`, use Codex's skill installer to select that skill from the official OpenAI skills repository. Preserve the skill's approval requirement. [R02](SOURCES.md#r02) [R20](SOURCES.md#r20)
